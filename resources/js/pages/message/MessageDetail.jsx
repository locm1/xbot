...
export default (props) => {
  ...
  const handleConfirmMessageClick = () => {
    getMessageItems(selectTemplate, setMessageItems).then(() => {
      setMessageDetailModal(true)
    })
  }
    ...
    <>
      ...
      <Card border="0" className="shadow message-detail-wrap mt-4">
        <Card.Header className="border-bottom d-flex align-items-center justify-content-between">
          ...
          <Button variant="primary" disabled={selectTemplate ? false : true} size="sm" onClick={handleConfirmMessageClick}>メッセージを確認</Button>
        </Card.Header>
        <Card.Body>
          <Row>
