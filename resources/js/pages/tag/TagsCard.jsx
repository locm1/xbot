    ...
    <>
      ...
      <Card.Body className="d-sm-flex align-items-center flex-wrap flex-lg-nowrap py-0">
        ...
        <Col xs={11} lg={9} className="px-0 mb-4 mb-md-0">
          {
            isEdit ? (
              <div className="mb-2 w-50">
                <Form>
                  <Form.Control required type="text" name="name" value={name} onChange={(e) => editName(e)} placeholder="タグ名を入力してください" />
                </Form>
                <div className="d-flex justify-content-end flex-wrap flex-md-nowrap align-items-center">
                  <div className="tag-edit-button ms-2 mt-2"><CheckIcon className="icon icon-xs" /></div>
                  <div onClick={() => setIsEdit(!isEdit)} className="tag-edit-button ms-2 mt-2"><Button variant="close" /></div>
                </div>
              </div>
            ) : (
              <div className="mb-2">
                <h6 className="">
                  {name}
                  <PencilIcon className="icon icon-xs ps-2 pb-1 management-tag-name" onClick={() => setIsEdit(!isEdit)} />
                </h6>
              </div>
            )
          }
        ...
        <Col xs={10} sm={2} lg={2} xl={2} className="d-none d-lg-block d-xl-inline-flex align-items-center ms-lg-auto text-right justify-content-center px-md-0">
          <div className="d-block d-sm-flex">
            <div className="ms-sm-3">
              <TrashIcon onClick={() => deleteTagsCard(id)} role="button" className="icon icon-xs text-danger me-2 "></TrashIcon>
            </div>
          </div>
        </Col>
      </Card.Body>
